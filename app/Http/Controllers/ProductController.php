...
if($request->multi_img != null){
  ...
  foreach ($image as $key => $value) {
   ...
   DB::table('product_attr_img')->insert(['product_id' => $pid, 'multi_image' => $fileName]);
   $value->move(public_path('product_attr_img'), $fileName);
 }
}
...
public function edit_data(Request $request, $id)
{
  ...
  $data['product_attr_img'] = DB::table('product_attr_img')->where('product_id',$id)->get();
